# Funciones de segmentacion adaptada, estos procesos se realizan a partir de un SPC de entrada
f0201_segmentacionAdaptada <- function(spc){
  ...
  layersSpline <- ithir::ea_spline(layers, var.name = "organic_carbon",
                            d = t(c(0, 5, 15, 30, 60, 100, 200)),
                            lam = 0.1, vlow = 0, vhigh = 200,
                            show.progress = TRUE)
  ...
  spc2 <- aqp::slice(spc2, 0:150 ~ organic_carbon)
