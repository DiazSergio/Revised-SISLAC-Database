...
f0101_validarErrores <- function(json){
  ...
  for (i in 1: NROW(json)) {
    ...
    jsonProfile$profile_id <- profileId
    id <- profileId
    jsonProfile$layer_identifier <- NULL
    ...
    for (j in 1: NROW(jsonProfile)) {
      if (isTRUE(duplicatedLayers[j]) & !isTRUE(duplicatedAttributes[j])) {
  ...
  for (i in 1: NROW(json)) {
    ...
    for (j in 1: (NROW(jsonProfile))) {
      if (((is.na(jsonProfile$bottom[j])) & (is.na(jsonProfile$top[j]))) |
          (NROW(jsonProfile) == 1 & ((is.na(jsonProfile$top[1]))|(is.na(jsonProfile$bottom[1]))))) {
  ...
  for (i in 1: NROW(json)) { #PRIMERO
    profileId <- json$properties.identifier[i]
    jsonProfile <- as.data.frame(json$properties.layers[i])
    id <- profileId
    
    if (NROW(jsonProfile) > 1) { #SEGUNDO
      for (j in 2: NROW(jsonProfile)) {
        if  (!is.na(jsonProfile$top[j]) & !is.na(jsonProfile$bottom[j]) 
             & !is.na(jsonProfile$top[j-1]) & !is.na(jsonProfile$bottom[j-1])){
          if (j == 2 & jsonProfile$bottom[j-1] != 0) {
            if ((jsonProfile$top[j] > jsonProfile$top[j-1] & jsonProfile$top[j] < jsonProfile$bottom[j-1]) |
                (jsonProfile$bottom[j] > jsonProfile$top[j-1] & jsonProfile$bottom[j] < jsonProfile$bottom[j-1])){
...
f0102_removerErrores <- function(json, errores){
  json <- json[!json$properties.identifier %in% errores$profileId,]
  return(json)
}

# f0103 - Validar perfiles con inconsistencias.
f0103_validarInconsistencias <- function(json){
  inconsistencias <- data.frame(id = character(), profileId = character(), status = character(),
                       errorCode = numeric())
  ...
  for (i in 1: NROW(json)) {
    ...
    jsonProfile <- as.data.frame(json$properties.layers[i])
    id <- profileId
    if (!is.na(jsonProfile$top[1]) & !is.na(jsonProfile$bottom[1]) & NROW(jsonProfile) > 1){
  ...
  for (i in 1: NROW(json)) {
    ...
    if (maxLayer == 1 & (!is.na(jsonProfile$top[1]) & !is.na(jsonProfile$bottom[1]))) {
      id <- jsonProfile$layer_identifier[1]
      ...
      for (j in 1: (NROW(jsonProfile))) {
        ...
        if (j == 1) {
          ...
          id <- jsonProfile$layer_identifier[j]
  ...
  #Validacion 07, horizontes final continuo
  for (i in 1: NROW(json)) {
    profileId <- json$properties.identifier[i]
    jsonProfile <- as.data.frame(json$properties.layers[i])
    id <- profileId
...
f0105_crearSPC <- function(json, inconsistencias){
  ...
  if(is.data.frame(spc)) {
    ...
    spc[3:4] <- lapply(spc[3:4], as.numeric)
    spc[6] <- lapply(spc[6], as.numeric)
