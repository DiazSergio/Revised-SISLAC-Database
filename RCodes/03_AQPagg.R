...
f0301_segmentacionAQP <- function(spc){
...
f0302_agregacionAQP <- function(spc){
  ...
  aggAQP <- aqp::slab(spc1, fm= ~ organic_carbon)
  return(aggAQP)
}
  
# funcion f0303 ploteo de la gráfica de agregación.
f0303_ploteoAgregacion <- function(aggAQP){
  ...
  plotAgg <- xyplot(top ~ p.q50 | variable, data=aggAQP, ylab='Depth (in cm)',  
                    xlab='median limited by the 25th and 75th percentiles',
